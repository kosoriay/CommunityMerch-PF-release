...
export function PricingForm({
  ...
  const [selectedColors, setSelectedColors] = useState<Record<string, string[]>>(
    () => Object.fromEntries(
      (initialSelectedIds as string[]).map((id) => [id, initialColors?.[id] ?? ["White"]])
    )
  )
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center">
            ...
            <Input
              ...
              min="1"
              step="1"
              defaultValue={initialGoal ? initialGoal / 100 : ""}
              className="rounded-l-none"
              placeholder="500"
            />
          </div>
          {selectedIds.length > 0 && minProfit > 0 && initialGoal && (
            <p className="text-xs text-muted-foreground">
              ~{itemsNeededForGoal(initialGoal, minProfit)} items needed to reach goal
            </p>
          )}
        </div>
        <div className="space-y-1">
          ...
          <Input
            ...
            name="deadline"
            type="date"
            defaultValue={initialDeadline ? initialDeadline.toISOString().split("T")[0] : ""}
          />
        </div>
      </div>
