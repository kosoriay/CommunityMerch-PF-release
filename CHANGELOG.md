...
> ファイルの同期（Sync fork）では Printful・Turso・Vercel の設定は変わりません。

### Fixed

- **Printful が受け付けた後で止めた注文が、管理画面で「成功」に見えていた問題を修正しました**

  Printful は、支払い方法が登録されていないなどの理由で、注文をいったん受け付けたあとで「Failed（失敗）」にすることがあります。これまでアプリは受け付けた時点で成功として扱い、管理画面にも成功した注文として表示し、購入者には確認メールまで送っていました。**実際には何も印刷されていませんでした。**

  今後は Printful 側の状態を、Webhook と毎日0時（UTC）の定期照会の両方で確かめます。止まっている注文は管理ダッシュボード最上段の **Needs attention** に「Stopped at Printful」として出て、運営者にメールが届きます。**アップデート後、最初の定期照会までの最大1日は、既存の注文が「Printful status not confirmed」として表示されます。まだ確認していないのは事実なので、正しい表示です。**

- **Stripe が同じ通知を再送すると、発送済み・返金済みの注文が「支払い済み」に戻り、もう一度発注処理に入っていた問題を修正しました**（確認メールも再送されていました）

- **確認メールの送信に失敗すると、その注文が要対応一覧にも再送の対象にも入らない状態に固まっていた問題を修正しました**

- **返金済みの注文にも、管理画面に Retry ボタンが出ていた問題を修正しました**

- **購入者の注文ページが、Printful の状態にかかわらず「Order confirmed! A confirmation has been sent」と表示していた問題を修正しました** — 確認メールを送ったかどうかをこのページは知りません。今後は「Payment received」「In production」「Shipped」「Refunded」の段階で表示します。**Printful 側の失敗は購入者には見せません**（運営者が裏で直せるため）

### Added

- **管理ダッシュボードの Needs attention を4つに分けました**: Not sent to production（既存）/ Paid but never sent to Printful / Stopped at Printful / Printful status not confirmed。それぞれ直し方を表示します
- **未発送の注文の返金パネルに「先に Printful で取り消す」注意を出すようにしました** — アプリの返金は購入者へのお金を戻すだけで、Printful の注文には何もしません。先に返金すると、Printful はそのまま印刷・発送し、製造費を請求します
- **団体の注文一覧のバッジを、購入者と同じ段階表示にしました**

### Changed

- **🔴 Printful の Webhook を9種類で登録し直してください（1回だけ。curl 1本）**

  状態の変化を知らせる6種類（`order_failed` ほか）を追加しました。**登録し直さなくても、翌日から毎日の定期照会が拾います。** ただし気付くのが最大1日遅れます。手順は `docs/2-setup/00-START-HERE.md` の 4-3「画面が見つからない場合」の手順1〜4。**9つを1回で送ってください**（置き換えなので、1つずつ送ると最後の1つしか残りません）。

  使っている Printful トークンに **webhooks の閲覧と管理**の権限が無いと、手順1が 403 で止まります。その場合は同じ節の「既にトークンを発行済みで、webhook スコープが無い場合」に従ってください。**注文の閲覧**の権限が無いと、状態の照会が 403 になり、すべての注文が「Printful status not confirmed」に出て、運営者に「トークンの失効を疑え」というメールが届きます。手順書 1-5 の手順6の一覧でトークンの権限を確認してください。

- **🔴 プレビュー専用のデータベースを分けていなければ、分けてください（v1.20.1 の手順）**

  このアップデートは注文の表に列を足します。データベースが1つのままだと、**このアップデートを受け取る前に作ったブランチのプレビューが後からビルドされたとき、本番の表から足した列が削除されます。** 手順は `docs/2-setup/00-START-HERE.md` の 4-5。プルリクエストを一度も作らない運用なら、実際には起きません。
...
- **キャンペーンページ上部の大きなデザイン画像が、古いキャンペーンで正しく表示されない問題を修正しました**

  v1.21.0 で商品カードの画像は自動的に直るようにしましたが、**ページ上部の大きなデザイン画像だけが対象外**でした。v1.21.0 より前に作られたキャンペーンでは、この画像の参照先が無効なまま残っていました。

  公開ページでは団体がアップロードしたデザイン画像が代わりに表示されるため、閲覧者に壊れた画像が見えることはありませんでした。ただし**ページを開くたびに必ず失敗する読み込みが1回発生**しており、さらに**団体側の管理画面（デザインの手順）では、画像が読み込めないアイコンが表示されたまま**になっていました。

  夜間のバッチ処理が、無効になった参照を片付けるようにしました。**団体側で何か操作していただく必要はありません。** 片付けが済んだキャンペーンでは、上部にはアップロードされたデザイン画像がそのまま表示され、管理画面の読み込めないアイコンも出なくなります。
...
- **「選択した画像が見えなくなる」問題を修正しました**

  キャンペーンの締め切りが過ぎたあと再オープンすると、商品カードの画像が消えて見える報告がありました。**原因はキャンペーンを再オープンしたことではありません。** 画像はそれより前から見えなくなっており、再オープンは団体がたまたまそのタイミングで画面を開き直しただけです。締め切り中ずっと同じ状態だったキャンペーンでも、次に画面を開けば同じことが起きていました。

  中身は、商品画像を生成した直後に Printful から返ってくる URL を**そのまま**データベースに保存していたことです。この URL は Printful 側の一時的な保管場所を指しており、恒久的なものではありません。実際に確認したところ、生成からおよそ10日で無効になっている画像がありました。Printful 側もこの URL がいつまで有効かを保証していません。無効になった時点でページを開いた人には、その商品の画像が表示されなくなります（通常のカタログ写真や、団体がアップロードしたデザイン画像は別の場所に保存されているため、これらは影響を受けません）。

  **このバージョンを導入すると、商品カードの壊れた画像リンクは自動的に直ります。** 新しく生成する画像は当方の保管場所に複製して恒久化するようにし、さらに夜間のバッチ処理が壊れた画像リンクを見つけ次第、複製済みの画像に自動で貼り替えます。団体側で何か操作していただく必要はありません。

  **キャンペーンページの上部に出る大きなデザイン画像は、この自動修復の対象ではありません。** こちらは、次にそのキャンペーンのデザインの手順を保存し直したときに置き換わります。それまでの間は、団体がアップロードしたデザイン画像を代わりに表示します。

- **商品画像が読み込めないときは、通常のカタログ写真を表示するようにしました**

  従来は商品画像の読み込みに失敗すると、その画像だけを非表示にしていました。カードには何も残らず灰色の空欄になるため、まるで画像そのものが消えてしまったかのように見えていました。今後は読み込みに失敗した場合、デザインの乗っていない通常のカタログ写真を代わりに表示します。カードには常に商品の写真が表示されます。

- **キャンペーンを再オープンすると、商品画像もその場で作り直すようにしました**

  従来はキャンペーンを再オープンしてもキャンペーン自体の状態が戻るだけで、商品画像のリンクはそれまでのままでした。そのため、すでに壊れていた画像を抱えたまま再オープンされることがありました。今後は再オープンした時点でその場で画像を作り直します。夜間のバッチ処理が動くのを待つ必要はありません。

- **デザインの手順で保存するプレビュー画像を、保存前に確認するようにしました**

  従来は保存する画像の保存場所を確認せずに受け付けていました。今後は当方が管理する保存場所にある画像だけを受け付けます。これにより、任意の画像アドレスを送り込んで公開ページに表示させてしまう経路を塞ぎました。
