...
ところが Vercel は、**本番だけでなくプレビュー（プルリクエストごとの試験公開）でも同じビルドを実行します。**
データベースが1つしかないと、こうなります:

- **プルリクエストを出しただけで、稼働中のストアのデータベース構造が変わる。** 誰もレビューする前に
- **プレビュー環境から、買い手の氏名・メールアドレス・配送先住所が読める。** 本番と同じデータを見ているため

データベースを2つに分ければ、どちらも起きません。**本番は本番のデータだけを持ち、プレビューは
自分で作った空のデータから始まります。**

> **「プレビューは使わない」場合も分けてください。** プレビューは、あなたが Sync fork でアップデートを
> 受け取ったときや、GitHub 上で何かを編集したときにも自動で作られます。**使うつもりが無くても走ります。**
...
本番運用を始めた後、この作業をしていないと、**プレビュー環境で買い物をしたときに実際のカードへ課金され、Printful に本物の注文が入って印刷・発送・請求まで進みます。** プレビューは、あなたがプルリクエストを出したときや Sync fork でアップデートを受け取ったときに自動で作られます。
...
> 💡 `STRIPE_WEBHOOK_SECRET` はプレビュー用を用意できません。Webhook はURLを1つ登録する仕組みですが、プレビューのURLはデプロイごとに変わるためです。**プレビューでは決済completion後の処理（注文確定・メール送信）が動かない**と理解しておいてください。画面の動作確認には支障ありません。
...
A: 動きます。ただし**プルリクエストを出したり Sync fork でアップデートを受け取ったりするたびに、稼働中のストアのデータベース構造が自動で書き換わり**、さらに**プレビューの画面から買い手の氏名・メールアドレス・配送先住所が読めます**。Turso の無料枠は**データベース100個まで**なので、2つ目を作っても費用は増えません。**実運用するなら分けてください。**
